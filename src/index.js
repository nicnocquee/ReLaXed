...
program
  ...
  .option(
    "-l, --locals <json>",
    "Json locals for pug rendering, string or path to .json file"
  )
...
if (options.temp) {
  var validTempPath =
    fs.existsSync(options.temp) && fs.statSync(options.temp).isDirectory();
  ...
  if (validTempPath) {
    ...
    console.error(
      colors.red(
        "ReLaXed error: Could not find specified --temp directory: " +
          options.temp
      )
    );
    process.exit(1);
...
async function build(filepath) {
  ...
  if (
    !updatedLocals &&
    !relaxedGlobals.watchedExtensions.some((ext) => filepath.endsWith(ext))
  ) {
  ...
  if (relaxedGlobals.busy) {
    console.log(
      colors.grey(`File ${shortFileName}: ignoring trigger, too busy.`)
    );
    return;
  }
  ...
  if (generatingPDF && relaxedGlobals.config.after && !generateError) {
    ...
    var subprocess = exec(
      relaxedGlobals.config.after,
      (cwd = relaxedGlobals.basedir)
    );
